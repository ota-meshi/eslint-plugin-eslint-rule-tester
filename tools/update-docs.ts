...
class DocFile {
	...
	public constructor(rule: { rule: Rule.RuleModule; meta: RuleModuleEnhancedMeta }) {
		this.rule = rule.rule;
		this.ruleId = rule.meta.ruleId;
		this.filePath = path.join(ROOT, `${rule.meta.ruleId}.md`);
		this.content = fs.readFileSync(this.filePath, 'utf8');
		this.since = pickSince(this.content);
	}

	public static read(rule: { rule: Rule.RuleModule; meta: RuleModuleEnhancedMeta }) {
		return new DocFile(rule);
	}

	public updateHeader() {
		const { fixable, hasSuggestions, deprecated, replacedBy, docs } = this.rule.meta!;
		const { description, recommended } = docs!;
		...
		if (deprecated) {
			if (replacedBy) {
				const replacedRules = replacedBy.map(
					(name) => `[eslint-rule-tester/${name}](${name}.md) rule`
				);
				notes.push(
					`- :warning: This rule was **deprecated** and replaced by ${formatItems(replacedRules)}.`
				);
			} else {
			...
			if (recommended) {
				notes.push('- :gear: This rule is included in `"plugin:eslint-rule-tester/recommended"`.');
			}
		}
		if (fixable) {
			notes.push(
...
This rule was introduced in eslint-plugin-eslint-rule-tester ${await this.since}

`
				: ''
		}## :mag: Implementation

- [Rule source](https://github.com/ota-meshi/eslint-plugin-eslint-rule-tester/blob/main/lib/rules/${this.ruleId}.js)
- [Test source](https://github.com/ota-meshi/eslint-plugin-eslint-rule-tester/blob/main/tests/lib/rules/${this.ruleId}.js)
`;
