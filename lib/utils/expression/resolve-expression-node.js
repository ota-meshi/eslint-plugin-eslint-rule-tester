	...
	const operations = {
		...
		CallExpression(node) {
			...
			if (
				...
				(isRequireIdentifier(node.callee.object) ||
		...
		TemplateLiteral(node) {
			...
			if (result.expressions.every((e) => e.type === 'Literal')) {
				const str = `${result.quasis[0].value.cooked}${result.expressions.map((e, i) => `${/** @type {SimpleLiteral} */ (e).value}${result.quasis[i + 1].value.cooked}`)}`;
				return {
					type: 'Literal',
					value: str
				};
