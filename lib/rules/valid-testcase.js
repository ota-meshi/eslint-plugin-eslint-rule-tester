...
module.exports = {
	...
	create(context) {
		...
		/** @type {import("../utils/types").LinterWorkerServise|null} */
		...
		function extractTestConfig(node) {
			...
			for (const prooerty of node.properties) {
				...
				const value = /** @type {Expression} */ (prooerty.value);
				if (name === 'errors') {
					errors = value;
				} else if (name) {
			...
			'CallExpression > MemberExpression.callee'(node) {
				...
				if (resolveRuleTester.node.type === 'NewExpression') {
					...
					if (arg && arg.type === 'ObjectExpression') {
						ruleTesterConstructorParams = resolveExpressionNode(/** @type {any} */ (arg), context);
					}
				}
			},
			/** @param {Property} node */
...
/**
 * @param {import('../utils/types').Suggestion} seggestion
 */
