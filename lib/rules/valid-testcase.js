...
module.exports = {
	...
	create(context) {
				...
				context.report({
					...
					fix: (fixer) => {
						if (message.messageId) {
							return fixToAddProperty(fixer, node, 'messageId', JSON.stringify(message.messageId));
						}
						return fixToAddProperty(fixer, node, 'message', JSON.stringify(message.message));
					}
				});
			}
		...
		function* fixToAddProperty(fixer, node, key, value) {
			if (node.properties.length === 0) {
				...
				if (node.loc.start.line < node.loc.end.line) {
					const indent = getTokenNextIndent(node);
					insertText = `\n${indent}`;
				}
			...
			if (node.loc.start.line === lastProperty.loc.end.line) {
				...
				const indent = getTokenIndent(lastProperty);
				insertText += `\n${indent}`;
			}
			insertText += `${key}: ${value}`;
		 ...
		 * @returns {IterableIterator<ReportFix>}
		 */
		function* fixToAddElement(fixer, node, element) {
			...
			if (node.loc.start.line === lastElementToken.loc.end.line) {
				...
				const indent = getTokenIndent(lastElementToken);
				insertText += `\n${indent}`;
			}
			insertText += element;
		...
		function* fixForMismatchErrorsLength(fixer, testcaseErrors, resultErrors) {
			if (testcaseErrors.elements.length > resultErrors.length) {
				for (const error of testcaseErrors.elements.slice(resultErrors.length)) {
					if (!error) continue;
					const maybeComma = sourceCode.getTokenBefore(/** @type {ESTreeNode} */ (error));
					yield fixer.removeRange([
						(maybeComma?.value === ',' ? maybeComma : error).range[0],
						error.range[1]
					]);
				}
				...
				yield* fixToAddElement(fixer, testcaseErrors, text);
			}
		}
		...
		function getTokenIndent(token) {
			return sourceCode.lines[token.loc.start.line - 1]
				.slice(0, token.loc.start.column)
				.replace(/\S/gu, ' ');
		}
		 ...
		 * @param {import('estree').SourceLocation} token.loc
		 */
		function getTokenNextIndent(token) {
			const indent = getTokenIndent(token);
...
function suggestionToString(suggestion) {
	return `{ ${
		suggestion.messageId
			? `messageId: ${JSON.stringify(suggestion.messageId)}`
			: `desc: ${JSON.stringify(suggestion.desc)}`
	}, output: ${outputToString(suggestion.output)} }`;
}
