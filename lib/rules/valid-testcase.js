...
module.exports = {
	meta: {
		docs: {
			...
			recommended: true,
			url: 'https://github.com/ota-meshi/eslint-plugin-eslint-rule-tester/blob/v0.0.0/docs/rules/valid-testcase.md'
		},
		fixable: 'code',
		messages: {
			...
			missingSuggestionOutput: "Test must specify 'output'."
		},
		schema: [],
		type: 'suggestion'
	},
	...
	create(context) {
			...
			if (!messageIdNode && !descNode) {
				context.report({
					...
					fix: (fixer) => {
						...
						return fixForAddProperty(fixer, node, 'desc', JSON.stringify(suggestion.desc));
					}
				});
			}
			if (!outputNode) {
				context.report({
					...
					fix: (fixer) => {
						return fixForAddProperty(fixer, node, 'output', outputToString(suggestion.output));
					}
				});
			}
			for (const item of [
				...
				if (result.suggestions?.length > 0) {
					if (test.type === 'Literal') {
						context.report({
							...
							fix: (fixer) => fixForAddSuggestion(fixer, test, result.suggestions)
						});
				...
				for (const def of variable.defs) {
					...
					if (def.type === 'Variable') {
						...
						if (
							...
							decl.init.parent.type === 'MemberExpression' &&
							getPropertyName(decl.init.parent) === 'require' &&
		 ...
		 * @param {Expression} node
		 * @param {import('../utils/linter-worker-servise/types').Suggestion[]} seggestions
		 * @returns {IterableIterator<ReportFix>}
		 */
		function* fixForAddSuggestion(fixer, node, seggestions) {
			if (node.type === 'Literal') {
				yield fixer.replaceText(
					node,
					`{ message: ${node.raw}, seggestions: [${seggestions.map(suggestionToString)}] }`
				);
				return;
			}
			if (node.type === 'ObjectExpression') {
				yield* fixForAddProperty(
					fixer,
					node,
					'seggestions',
					`[${seggestions.map(suggestionToString)}]`
				);
			}
		}
		 ...
		 * @returns {IterableIterator<ReportFix>}
		 */
		function* fixForAddProperty(fixer, node, key, value) {
			...
			const targetToken = before?.value === ',' ? before : closeBrace;
			if (!targetToken) return;
			yield fixer.insertTextBeforeRange(targetToken.range, `,${key}: ${value}`);
		}
